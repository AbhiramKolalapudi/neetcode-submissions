...
private:
    ...
    TreeNode* deserial_dfs(vector<string>& vals, int& i)
    {
        ...
        newnode->right = deserial_dfs(vals, i);//so basically the recursion continues to build the same tree until an N comes after which the fn returns and moves to next branch
