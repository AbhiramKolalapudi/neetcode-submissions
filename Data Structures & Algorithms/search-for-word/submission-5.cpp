...
private:
    bool dfs(vector<vector<char>>& board, string word, int r, int c, int i)
    {
        ...
        bool res = ( dfs(board, word, r + 1, c, i + 1) || dfs(board, word, r - 1, c, i + 1) || 
                     dfs(board, word, r, c + 1, i + 1) || dfs(board, word, r, c - 1, i + 1) ); //now see if the next letter(i + 1) exists anywhere in r +- 1 or c +- 1.

        board[r][c] = word[i];//unmark the letter after the search.
