...
private:
    ...
    bool safe(vector<string>& board, int pos)
    {
        ...
        int leftdiag = pos - n; //we basically check the if queen exits diagonally left and right and their index will start from current position - board size (for left) and current position + board size (for right)
        int rightdiag = pos + n;
