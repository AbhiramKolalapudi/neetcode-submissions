...
private:
    ...
    void dfs(vector<vector<char>>& board, TrieNode* node, string& word, vector<string>& res,  int r, int c)
    {
        ...
        node = node->children[idx]; //go down a level in the trie to the letter (board[r][c]) from current node
        ...
        dfs(board, node, word, res, r - 1, c); //call dfs for all sides of the board to check the next letter
        ...
        dfs(board, node, word, res, r, c + 1);
        board[r][c] = word.back(); //backtrack by unmarking the letter on board and popping a letter from word string
        word.pop_back();
    }
